...
void Star::Render(double zoom, float window_x)
{
    // First, render all child planets
    for (Planet &planet : planets) planet.Render(zoom, window_x);
    ...
    float min_zoom = radius / (0.1 * window_x); // Zoom level at which the star covers 10% of the screen
